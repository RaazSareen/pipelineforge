...
from pipelineforge.evaluation.classification import compute_all_classification_metrics
...
from pipelineforge.models.classifier import build_classifier, train_classifier
...
def run_classification_pipeline(
    ...
    """
    Run an end-to-end classification pipeline.
    ...
    model_type : str
        Classifier registry key. Default: 'random_forest'.
    test_size : float
        Fraction of data for test set.
    random_state : int
        Random seed for reproducibility.
    model_kwargs : dict | None
        Additional kwargs passed to the classifier constructor.
    ...
    logger.info(
        "run_classification_pipeline: model=%s | target=%s | test_size=%.2f",
        model_type,
        target_column,
        test_size,
    )
    ...
    metrics = compute_all_classification_metrics(y_test, predictions)

    logger.info("run_classification_pipeline: metrics=%s", metrics)
